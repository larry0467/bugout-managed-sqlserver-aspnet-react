...
public record PromoteRequestDto
{
    ...
    [JsonPropertyName("fromBranch")] public string FromBranch { get; init; } = "";
    [JsonPropertyName("toBranch")] public string ToBranch { get; init; } = "";
    [JsonPropertyName("tagOnPromote")] public bool TagOnPromote { get; init; }
...
public interface IClaudeAgentClient
{
    ...
    Task<PromoteResultDto> PromoteAsync(PromoteRequestDto request, CancellationToken ct);
}
...
public class ClaudeAgentClient : IClaudeAgentClient
{
    ...
    public async Task<ClaudeRunResultDto> RunAsync(ClaudeRunRequestDto request, CancellationToken ct)
    {
        try
        {
            // Sidecar route is POST /run (singular). Don't pluralize.
    ...
    public async Task<PromoteResultDto> PromoteAsync(PromoteRequestDto request, CancellationToken ct)
    {
        try
        {
            var resp = await _http.PostAsJsonAsync("promote", request, ct);
            if (!resp.IsSuccessStatusCode)
            {
                var body = await resp.Content.ReadAsStringAsync(ct);
                _log.LogWarning("Sidecar /promote returned {Status}: {Body}", resp.StatusCode, body);
                return new PromoteResultDto
                {
                    Error = $"Sidecar HTTP {(int)resp.StatusCode}: {body}",
                };
            }
            var result = await resp.Content.ReadFromJsonAsync<PromoteResultDto>(cancellationToken: ct);
            return result ?? new PromoteResultDto { Error = "Sidecar returned empty body" };
        }
        catch (Exception ex)
        {
            _log.LogError(ex, "ClaudeAgentClient promote call failed for {Owner}/{Repo} {From}->{To}",
                request.Owner, request.Repo, request.FromBranch, request.ToBranch);
            return new PromoteResultDto { Error = "Sidecar promote exception: " + ex.Message };
        }
    }
}
