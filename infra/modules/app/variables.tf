...
variable "sidecar_image" {
  description = "Claude Agent sidecar image (Node.js). Co-located in the same Container App as the API. The two share localhost so the API's BugsManaged:ClaudeAgentSidecar:Url default of localhost:7100 just works without extra config."
  type        = string
  default     = "mcr.microsoft.com/k8se/quickstart:latest"
}

variable "sidecar_enabled" {
  description = "Whether to add the Claude Agent sidecar container to the Container App template. Off by default; turn on in any env where assign-to-claude is exercised (i.e. wherever anthropic_enabled is true)."
  type        = bool
  default     = false
}
