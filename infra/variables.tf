...
variable "sidecar_image" {
  description = "Claude Agent sidecar image (Node.js). Co-located in the same Container App as the API; both share localhost so the API's BugsManaged:ClaudeAgentSidecar:Url default of localhost:7100 just works."
  type        = string
  default     = "mcr.microsoft.com/k8se/quickstart:latest"
}
