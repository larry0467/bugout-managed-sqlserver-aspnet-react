...
public class OwnerApprovalLoopTests
{
    ...
    private static (TicketController controller, BugsManagedDbContext db, FakeOrgContext org) BuildController(
        string callerEmail = "owner@bugout.example",
    ...
    // the ticket to OWNER_APPROVAL_PENDING with claude@bugout.example as
    ...
    public async Task ClaudeAutoFlip_SuccessWithPr_FlipsToApprovalPending()
    {
        ...
        var t = SeedTicket(db, "ASSIGNED_CLAUDE", assignedTo: "claude@bugout.example");
            ...
            && t.EscalationStage == "ASSIGNED_CLAUDE")
        {
            ...
            t.SubmittedForApprovalBy = "claude@bugout.example";
            t.Status = "IN_REVIEW";
        ...
        Assert.Equal("claude@bugout.example", refreshed.SubmittedForApprovalBy);
